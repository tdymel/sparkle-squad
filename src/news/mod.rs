...
use dioxus::prelude::*;
use libero::{
    components::{Box, Divider, Flex, Image, List, ListItem, Text, Title},
    sx::sx,
    theme::Size,
};
...
use assets::{SLIDESHOW_IMAGES, THUMBNAILS, THUMBNAILS_FULL};
    ...
    rsx! {
        section {
            ...
            Flex {
                direction: "column",
                gap: "lg",
                Title { id: "news", variant: "h2", size: "h1", "{i18n.news().title()}" }
                List {
                    sx: sx().gap("lg"),
                    for (i , article) in articles.iter().enumerate() {
                        Article {
                            thumbnail: THUMBNAILS[i],
                            thumbnail_full: THUMBNAILS_FULL[i],
                            title: article.title(),
                            date: article.date(),
...
fn Article(
    thumbnail: Asset,
    thumbnail_full: Asset,
    title: &'static str,
    date: &'static str,
    date_label: &'static str,
    description: &'static str,
) -> Element {
    let gallery = SLIDESHOW_IMAGES.get(date);
        ...
        ListItem {
            ...
            Flex {
                sx: sx()
                    ...
                    .align_items("stretch")
                    .gap("md")
                    .breakpoint(Size::Sm, sx().flex_direction("row").align_items("flex-start")),
                Box {
                    sx: sx()
                        .width("100%")
                        .max_width("26rem")
                        .align_self("center")
                        .flex_shrink("0")
                        .breakpoint(Size::Sm, sx().width("18rem").align_self("flex-start")),
                    Image {
                        src: thumbnail,
                        zoomed_src: thumbnail_full,
                        radius: "sm",
                        alt: title,
                        zoomable: true,
                        sx: sx().height("auto"),
                ...
                Flex {
                    ...
                    Flex {
                        direction: "column",
                        gap: "xs",
                        Title { id: date, variant: "h3", size: "h2", {title} }
                        Text {
                            size: "md",
                            span: true,
                    ...
                    if let Some(images) = gallery.filter(|images| images.len() > 1) {
                        Flex {
                            direction: "row",
                            wrap: true,
                            gap: "sm",
                            for image in images.iter().skip(1) {
                                Box {
                                    sx: sx().width("96px").height("96px").flex_shrink("0"),
                                    Image {
                                        src: *image,
                                        fit: "cover",
                                        radius: "sm",
